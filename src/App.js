...
// ✅ 1. 核心幾何算法：移至最外層，確保 handleMouseUp 能正確呼叫，避免 ReferenceError
...
const CncWorkspace = () => {
  ...
  const visualCompPaths = useMemo(
    () => visualPaths.map((p) => ({ ...p, points: getOffsetPoints(p.points, toolConfig.r, isInner) })),
    [visualPaths, toolConfig.r, isInner]
  );
  const activeCompPts = useMemo(
    () => visualCompPaths[paths.findIndex((p) => p.id === activePath?.id)]?.points || [],
  ...
  const genGCode = () => {
    ...
    vPts.forEach((pt, i) => {
      if (i === 0) return;
      const cmd = pt.type === "arc" ? (pt.ccw ? "G03" : "G02") : pt.type || "G01";
    ...
    while ((isInner ? currentY < targetY : currentY > targetY) && passes < 150) {
      ...
      for (let i = 0; i < activeCompPts.length - 1; i++) {
        ...
        if (p1 && p2 && ((currentY >= p1.y && currentY <= p2.y) || (currentY <= p1.y && currentY >= p2.y))) {
          if (p1.y !== p2.y) endZ = p1.x + ((currentY - p1.y) / (p2.y - p1.y)) * (p2.x - p1.x);
          break;
        }
      }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (cur && typeof cur.x !== "undefined") {
        ...
        if (mode === "DRAW_LINE" && interactionRef.current.continuous && activePath?.points) {
          const lastPt = activePath.points[activePath.points.length - 1];
          if (lastPt) { ctx.save(); ctx.setLineDash([5/cameraRef.current.zoom]); ctx.strokeStyle = lineMode === "G00" ? "rgba(255,0,0,0.5)" : "rgba(0,255,0,0.5)"; ctx.beginPath(); ctx.moveTo(lastPt.x, lastPt.y); ctx.lineTo(end.x, end.y); ctx.stroke(); ctx.restore(); }
        } else if (mode === "DRAW_ARC") {
          const aps = interactionRef.current.arcPts;
          if (aps.length === 1) { ctx.beginPath(); ctx.moveTo(aps[0].x, aps[0].y); ctx.lineTo(end.x, end.y); ctx.stroke(); }
          else if (aps.length === 2) {
            try { const a = arcSubMode === "SCE" ? calcSCE(aps[0], aps[1], end) : calcSER(aps[0], aps[1], end); if (a) { ctx.beginPath(); ctx.arc(a.cx, a.cy, a.radius, a.startAngle, a.endAngle, a.ccw); ctx.stroke(); } } catch (e) {}
          }
        }

        const c = mode === "CHAMFER" ? "#ff00ff" : (interactionRef.current.isSnapped ? "#ffeb3b" : (mode === "SELECT" ? "#0dcaf0" : "#00ff00"));
        ctx.strokeStyle = c; ctx.lineWidth = 1 / cameraRef.current.zoom; ctx.beginPath();
      ...
      if (interactionRef.current.sim.active) {
        ...
        if (pt1 && pt2) {
          ...
          ctx.setLineDash(pt2.type === "G00" ? [3/cameraRef.current.zoom] : []);
          ctx.beginPath(); ctx.moveTo(pt1.x, pt1.y); ctx.lineTo(curX, curY); ctx.stroke(); ctx.setLineDash([]);

          ctx.fillStyle = toolConfig.color; ctx.beginPath();
          const aRad = (toolConfig.angle / 2) * (Math.PI / 180), dirY = isInner ? -1 : 1, hL = 20 / cameraRef.current.zoom;
          ctx.arc(curX, curY, toolConfig.r, 0, Math.PI * 2); ctx.fill();
          ctx.beginPath(); ctx.moveTo(curX, curY);
          ctx.lineTo(curX + Math.cos(aRad) * hL, curY + Math.sin(aRad) * hL * dirY);
          ctx.lineTo(curX + Math.cos(aRad) * hL + 15 / cameraRef.current.zoom, curY + Math.sin(aRad) * hL * dirY);
          ctx.lineTo(curX + Math.cos(aRad) * hL + 15 / cameraRef.current.zoom, curY - Math.sin(aRad) * hL * dirY);
          ctx.lineTo(curX + Math.cos(aRad) * hL, curY - Math.sin(aRad) * hL * dirY);
          ctx.fill();
        }
