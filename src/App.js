...
// ✅ 1. 搬移到最外層：解決「checkIntersect is not defined」報錯
...
const CncWorkspace = () => {
  ...
  // ✅ 2. 攝影機初始化為 0，由 useEffect 自動置中
  const cameraRef = useRef({ x: 0, y: 0, zoom: 1.5 });

  const interactionRef = useRef({
    isDragging: false, lastMouse: { x: 0, y: 0 }, continuous: false, currentPt: { x: 0, y: 0 },
    isSnapped: false, trimPath: [], circleStart: null, tangentCircle: null, arcPts: [],
  ...
  const genGCode = () => {
    if (!activePath || !activePath.points || activePath.points.length < 2) { alert("請畫出或選定一條包含2點以上的加工路徑！"); return; }
  ...
  const handleMouseDown = (e) => {
    ...
    const snapPt = { x: (e.clientX - rect.left - cameraRef.current.x) / cameraRef.current.zoom, y: (e.clientY - rect.top - cameraRef.current.y) / cameraRef.current.zoom };
    
    if (mode === "PAN") { interactionRef.current.isDragging = true; interactionRef.current.lastMouse = { x: e.clientX, y: e.clientY }; }
    else if (mode === "TRIM") { interactionRef.current.isDragging = true; interactionRef.current.trimPath = [snapPt]; }
    else if (mode === "DRAW_LINE") {
      saveState(); setPaths((prev) => {
