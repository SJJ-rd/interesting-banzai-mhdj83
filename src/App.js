...
const CncWorkspace = () => {
  ...
  // ✅ 2. 攝影機初始值設為 0，由 useEffect 自動計算置中
  ...
  const genGCode = () => {
    if (!activePath || !activePath.points || activePath.points.length < 2) { alert("請畫出路徑！"); return; }
  ...
  useEffect(() => {
    ...
    if (cameraRef.current.x === 0 && canvas.parentElement) {
      ...
      cameraRef.current.x = parentW / 2 - 150; // 考慮右側面板
    ...
    const render = () => {
      ...
      visualPaths.forEach((p, idx) => {
        ...
        if (cPts && Array.isArray(cPts) && !interactionRef.current.sim.active) {
    ...
    if (mode === "PAN") {
      ...
      setPaths((prev) => {
        ...
        if (!interactionRef.current.continuous || idx === -1) {
          const id = Date.now(); n.push({ id, points: [{ ...snapPt, type: lineMode, c: 0, r: 0 }] }); setSelectedPathId(id);
        } else { n[idx].points.push({ ...snapPt, type: lineMode, c: 0, r: 0 }); }
        return n;
      });
  ...
  const handleMouseUp = () => {
    if (mode === "TRIM" && interactionRef.current.trimPath.length > 1) {
      ...
      setPaths((prev) => {
        const allSegs = [];
        prev.forEach((p) => { if (p.points) for (let i = 0; i < p.points.length - 1; i++) if (p.points[i] && p.points[i + 1]) allSegs.push({ A: p.points[i], B: p.points[i + 1] }); });
        let newPaths = [];
        prev.forEach((path) => {
          if (!path.points) return; let cur = [];
          for (let i = 0; i < path.points.length - 1; i++) {
            const p1 = path.points[i], p2 = path.points[i + 1]; let cut = false;
            for (let j = 0; j < tPath.length - 1; j++) { if (checkIntersect(p1, p2, tPath[j], tPath[j + 1], true)) { cut = true; break; } }
  ...
  return (
    <div style={{ display: "flex", width: "100vw", height: "100vh", backgroundColor: "#1e1e1e", color: "white", overflow: "hidden" }}>
      <div style={{ flexGrow: 1, position: "relative" }}>
        <canvas ref={canvasRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onContextMenu={(e) => e.preventDefault()} style={{ display: "block", width: "100%", height: "100%", cursor: "crosshair" }} />
      ...
      <div style={{ width: "400px", background: "#252526", padding: "20px", borderLeft: "1px solid #444", overflowY: "auto" }}>
        ...
        <button onClick={genGCode} style={{ width: "100%", padding: "10px", background: "#28a745", color: "white", border: "none", borderRadius: "4px", fontWeight: "bold", marginBottom: "5px" }}>🚀 產生 G-Code</button>
        <button onClick={startSimulation} style={{ width: "100%", padding: "10px", background: "#17a2b8", color: "white", border: "none", borderRadius: "4px", fontWeight: "bold" }}>🎬 啟動模擬</button>
